...
class BertEmbeddings(nn.Module):
    ...
    def forward(self,
        ...
        if selection_mask is not None:
            embeddings = embeddings * selection_mask.unsqueeze(-1)
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        outputs = (
            attention_output,) + self_outputs[1:]  # add attentions if we output them
        return outputs
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        classifier_dropout = (config.classifier_dropout if config.classifier_dropout
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def forward(
        ...
        input_ids = input_ids.view(-1,
                                   input_ids.size(-1)) if input_ids is not None else None
